...
import { PiggyBank, Plus, CheckCircle2, AlertTriangle, ArrowUpRight, Loader2 } from 'lucide-react';
...
const Savings: React.FC<SavingsProps> = ({ savings, refreshData, members, settings }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Logic to update member's total savings could be a trigger, but let's refresh
      refreshData();
      setIsAdding(false);
    }
    setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold">Poupança</h2>
        <button 
        ...
        >
          <Plus size={20} />
          <span className="text-sm font-bold">Novo Depósito</span>
        </button>
      </div>
      ...
      <div className="bg-orange-50 border border-orange-100 p-4 rounded-2xl flex gap-3">
        ...
        <p className="text-xs text-orange-700 leading-tight">
          Atenção: Depósitos realizados após o dia 10 de cada mês sofrem multa automática de 15%.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {savings.length > 0 ? (
          savings.map(saving => {
            ...
            return (
              <div key={saving.id} className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm hover:border-[#aa0000]/20 transition-colors">
                ...
                <div className="text-right">
                  <p className="font-bold text-gray-800">{saving.amount.toLocaleString()} MT</p>
                  {saving.lateFee > 0 && (
                    <p className="text-[10px] text-red-500 font-bold uppercase tracking-tighter animate-pulse">+ {saving.lateFee.toLocaleString()} MT (Multa)</p>
                  )}
                </div>
              </div>
            );
          ...
          <div className="py-12 text-center bg-white rounded-3xl border border-dashed border-gray-200">
            <PiggyBank className="mx-auto text-gray-200 mb-2" size={40} />
            <p className="text-sm text-gray-400">Nenhum depósito registrado ainda.</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-[32px] p-8 animate-slide-up shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select 
                  className="w-full mt-1 px-4 py-3 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-[#aa0000]/20 font-medium"
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full mt-1 px-4 py-3 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-[#aa0000]/20 font-bold"
                  value={formData.amount}
                  onChange={e => setFormData({...formData, amount: Number(e.target.value)})}
                  required
                />
                <p className="text-[9px] text-gray-400 mt-1 ml-2">Mín: {settings.minMensalidade} | Máx: {settings.maxMensalidade}</p>
              </div>
              <div>
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" disabled={loading} className="flex-[2] py-4 bg-[#aa0000] text-white text-sm font-bold rounded-2xl shadow-lg shadow-[#aa0000]/20 active:scale-95 transition-all flex justify-center items-center">
                  {loading ? <Loader2 className="animate-spin" size={20} /> : 'Confirmar Depósito'}
                </button>
              </div>
            </form>
          </div>
